...
const addLanguage = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:5000/Condidate/${Userid}`).then((response) => {
        ...
        setcandidateData((prevData) => ({
            ...prevData,
            _id:userData.ALLcandidates[0]._id,
            dateOfBirth: userData.ALLcandidates[0].dateOfBirth,
            sex: userData.ALLcandidates[0].sex,
            address: userData.ALLcandidates[0].address,
            sessionId:userData.ALLcandidates[0].sessionId,
            
          }));
          setDiplomas(userData.diplomas || []);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const updateCandidateData={...candidateData};
      const updateExperiencesData=[...experiences];
      const updateDiplomas=[...diplomas];
      const updateLanguagesData=[...languages];
    console.log(candidateData._id);
    await axios.put(`http://localhost:5000/user/update/${Userid}`, updatedUserData);
   await axios.put(`http://localhost:5000/Condidate/updatecondidate/${candidateData._id}`, updateCandidateData);
    await axios.put(`http://localhost:5000/Experience/updatexperience`, updateExperiencesData);
   await axios.put(`http://localhost:5000/diploma/updatediploma`, updateDiplomas);
    await axios.put(`http://localhost:5000/language/updateLanguage`, updateLanguagesData);
  ...
  <MenuItem value="Associate Degree">Associate Degree</MenuItem>
        ...
        <Grid item xs={12} sm={6}>
          <TextField
            ...
            value={diploma.startDateDiploma ? diploma.startDateDiploma.toString().split('T')[0] : ''}
        ...
        <Grid item xs={12} sm={6}>
          <TextField
            ...
            type="date"
            value={diploma.endDateDiploma ? diploma.endDateDiploma.toString().split('T')[0] : ''}
            onChange={(e) => handleDiplomaChange(e, index)}
            InputLabelProps={{
              shrink: true,
            }}
    ...
    <Grid container spacing={3}>
      ...
      <Grid item xs={12} sm={6}>
        <TextField
          ...
          type="date"
          value={experience.startDate ? experience.startDate.toString().split('T')[0] : ''}
          onChange={(e) => handleExperienceChange(e, index)}
          InputLabelProps={{
            shrink: true,
          }}
      ...
      <Grid item xs={12} sm={6}>
        <TextField
          ...
          type="date"
          value={experience.endDate ? experience.endDate.toString().split('T')[0] : ''}
          onChange={(e) => handleExperienceChange(e, index)}
          InputLabelProps={{
            shrink: true,
          }}
